...
class DesignerController extends GetxController with WidgetsBindingObserver {
  var isLoading = true.obs;
  RxList<Clients> designerList = <Clients>[].obs;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for(int i = 0 ; i < tempclientlist.length; i++){
          if(tempclientlist[i].role == "Designer"){
            designerList.add(tempclientlist[i]);
          }
        }
      } else {
  ...
  Future<void> createDesigner() async {
    try {
  ...
  Future<void> updateDesigner() async {
    try {
      var url = Uri.parse('${AppStrings().BASE_URL}users');
  ...
  ClearData(){
    ...
    image.value = null;
    city.value = "-- No City Select --";
    state.value = "-- No City Select --";
    country.value = "-- No City Select --";
    cityId.value = 0;
    stateId.value = 0;
    countryId.value = 0;
    update();
