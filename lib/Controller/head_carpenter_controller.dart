...
class HeadCarpenterController extends GetxController with WidgetsBindingObserver {
  ...
  void getHeadCarpenters() async {
    headcarpenterList.clear();
    isLoading(true);
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for(int i = 0 ; i < tempclientlist.length; i++){
          if(tempclientlist[i].role == "Head Carpenter"){
  ...
  Future<void> createHeadCarpenter() async {
    try {
  ...
  Future<void> updateHeadCarpenter() async {
    try {
      var url = Uri.parse('${AppStrings().BASE_URL}users');
  ...
  void DeleteHeadCarpenter(id) async {
    try {
      var response =
  ...
  void getHeadCarpenterData(id) async {
    isLoading(true);
    try {
